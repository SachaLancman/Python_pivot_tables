...
def create_RIN_report():
    ...
    df_4['MTM_purchase_gbl'] = df_4.apply(
        lambda row: "" if row['purchase_gbl_amt $'] == 0 else row['Market_price'] - row['purchase_gbl_amt $'], axis=1)

    df_4['MTM_sale_gbl'] = df_4.apply(
        lambda row: "" if row['sale_gbl_amt $'] == 0 else row['sale_gbl_amt $'] - row['Market_price'], axis=1)
    ...
    currency_fields = ['purchase_due_amt $', 'sale_due_amt $', 'NET', 'Pay_risk $ (pty -8d)', 'Pay_risk_secured $ (pty-8d)',
                       ...
                       'Market_price', 'MTM_purchase_gbl', 'MTM_sale_gbl']
