...
url = 'https://aegis-energy.com/insights/lcfs-rin-pricing-report-through-july-21-2023'
...
# Create a column for the MTM, MTM_purchase_gbl, MTM_sale_gbl
df_5['MTM'] = df_5['Quantity'] * df_5['Price_Rin (b)']

df_5['MTM_purchase_gbl'] = df_5.apply(lambda row: "" if row['purchase_gbl_amt $'] == 0 else row['MTM'] - row['purchase_gbl_amt $'], axis=1)

df_5['MTM_sale_gbl'] = df_5.apply(lambda row: "" if row['sale_gbl_amt $'] == 0 else row['sale_gbl_amt $'] - row['MTM'], axis=1)
...
currency_fields = ['purchase_due_amt $', 'sale_due_amt $', 'NET', 'Pay_risk $ (pty -8d)','Price_Rin', 'Price_Rin (b)', 'Unit price$', 'purchase_gbl_amt $', 'sale_gbl_amt $', 'MTM', 'MTM_purchase_gbl','MTM_sale_gbl']
...
mtm_purchase_gbl_range.FormatConditions.Add(Type=1, Operator=5, Formula1="0")
...
mtm_purchase_gbl_range.FormatConditions.Add(Type=1, Operator=6, Formula1="0")
mtm_purchase_gbl_range.FormatConditions(mtm_purchase_gbl_range.FormatConditions.Count).SetFirstPriority()
mtm_purchase_gbl_range.FormatConditions(1).Interior.Color = 13421823  # Red

# Apply conditional formatting to "MTM_sale_gbl" column
mtm_sale_gbl_range.FormatConditions.Add(Type=1, Operator=5, Formula1="0")
...
mtm_sale_gbl_range.FormatConditions.Add(Type=1, Operator=6, Formula1="0")
mtm_sale_gbl_range.FormatConditions(mtm_sale_gbl_range.FormatConditions.Count).SetFirstPriority()
