...
sql_query = "SELECT id_counter, entity, profit_center, Deal#, Quality, Linkage_Execution, Deal_date, Credit_due_date, Credit_pty_trf, " \
            "Linkage#, Credit_Term, Trader, [sale_due_amt $], [purchase_due_amt $],[sale_due_amt $] - [purchase_due_amt $] AS [NET], [Pay_risk $ (pty -8d)] " \
            "FROM MED_BI.dbo.credit_phy_deal WHERE profit_center = 'Petrochemicals' AND Credit_pty_trf BETWEEN '2023-07-01' AND '2023-07-31'"
...
# Save table to Excel
df_4.to_excel('auto_petro_july.xlsx', index=False)
...
# Open the workbook
wb = xlApp.Workbooks.Open(r'C:\Users\SLANCM\PycharmProjects\python_Project_1\auto_petro_july.xlsx')
...
pt1.TableStyle2 = "PivotStyleDark4"
...
pt2.TableStyle2 = "PivotStyleDark4"
...
ws_report2.name = 'counterparties'

# Save the modified workbook
wb.Save()

print("Tables imported and pivot tables created in 'auto_petro_july.xlsx'")
