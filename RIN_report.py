...
df_5['MTM_purchase_gbl'] = df_5.apply(lambda row: "" if row['purchase_gbl_amt $'] == 0 else row['Price_market'] - row['purchase_gbl_amt $'], axis=1)

df_5['MTM_sale_gbl'] = df_5.apply(lambda row: "" if row['sale_gbl_amt $'] == 0 else row['sale_gbl_amt $'] - row['Price_market'], axis=1)
...
currency_fields = ['purchase_due_amt $', 'sale_due_amt $', 'NET', 'Pay_risk $ (pty -8d)','Price_Rin', 'Price_Rin (b)', 'Unit price$', 'purchase_gbl_amt $', 'sale_gbl_amt $', 'Price_market', 'MTM_purchase_gbl','MTM_sale_gbl']
